...
const getSeatGeekData = async () => {
  ...
  let page = 1;
  try {
    if (!Config.SEAT_GEEK_CLIENT_ID) throw new Error ("No Seat Geek Client ID found in Config.gs file");
    ...
    await getSeatGeekData()
      ...
      .then(async(data) => {
        ...
        for (let i=0; i<data.length; i++) {
          ...
          let date =  Utilities.formatDate(new Date(event.datetime_local), "PST", "yyyy/MM/dd HH:mm");
          // Logger.log(date);
          let url = event?.url;
          ...
          for (let j=0; j<artistList.length;j++) {
            ...
            for (let index=0; index<acts.length; index++) {
              ...
              if(res.toUpperCase() === artistList[j].toString().toUpperCase()) {
                Log.Info(`searchSeatGeek() - Found a match for artist: ${artistList[j]} in list of acts - event name: ${eName}`);
